...
var Zotlib = {
    ...
    createShortenedOverwrite: function (file, type, mode, maxBytes, item_id) {
        file = this.pathToFile(file);
        ...
        // Limit should be 255, but leave room for unique numbering if necessary
        // In this case, add enough padding to account for a space and ID (maybe 1 mil)
        var padding = 7;
        ...
        while (true) {
            ...
            try {
                ...
                // Modified
                else if (e.name == "NS_ERROR_FILE_ALREADY_EXISTS") {
                    return file.leafName;
                }
                else {
                    Zotero.debug("Path is " + file.path);
                ...
                // Shorten the filename
                //
                // Shorten the filename to support appending the id at the end
                var uniqueFile = file.clone();
                while (true) {
                    let newBaseName = uniqueFile.leafName.substr(0, newLength - ext.length);
                    // Modified
                    var newName = newBaseName + " " + item_id + ext;
